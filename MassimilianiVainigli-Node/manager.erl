...
% @param List_friends = lista amici
% @param List_blocks = propria visione della blockChain
% @param Not insert transactions = transactions ancora da esaminare
% @param Mining transactions = transactions sotto mining da questo nodo
manager(Friendship, BlockChain, Miner, List_friends, List_blocks, Not_inserted_transactions, Mining_transactions) ->
  ...
  case (length(Not_inserted_transactions) > 0 ) and (Miner =:= 0) of
    true ->
  ...
  receive
    ...
    {updateChain, Chain} ->
      ...
      case Miner =/= 0 of true -> exit(Miner, kill); false -> ok end,
    ...
    {push, Transazione} ->
      ...
      case length(NewTransitions) == 0 of
        true -> ok;
        false ->
    ...
    {stampa} ->
      io:format("Sono ~p. ~n Amici: ~p ~n Blocchi ~p ~n Trans non inserite: ~p ~n Sto minando ~p ~n",
        [self(), List_friends, List_blocks, Not_inserted_transactions, Mining_transactions])
