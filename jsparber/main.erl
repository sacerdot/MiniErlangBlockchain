...
msg_anomaly(Sender, Msg) ->
    X = rand:uniform(10),
    if % If sender is none then don't do anything
       Sender == none -> none;
       %Loose message
       X == 1 -> none;
       % Send message twice
       X == 10 -> Sender ! Msg, Sender ! Msg;
       % Send message without anomaly
       true -> Sender ! Msg
    end.
...
watch(Main, Node) ->
    spawn(fun () -> watch(Main, Node, ?DEAD_TOLERANZ) end).
...
watch(Main, Node, Toleranz) ->
    ...
    receive
      {pong, Ref} ->
	  sleep(10), watch(Main, Node, ?DEAD_TOLERANZ)
      after 2000 ->
		case Toleranz =< 0 of
		  true -> Main ! {dead, Node};
...
get_previous(Reply, Storage, Block_id) ->
    spawn(fun () ->
		  Nonce = make_ref(),
		  (?NODE) !
		    {ask, {get_previous, self(), Nonce, Block_id}},
		  receive
		    {previous, Nonce, none} ->
			io:format("Didn't find block~p~n", [Block_id]),
			Reply ! {result, none};
		    {previous, Nonce, Block} ->
			case check_block(Block) of
			  true ->
			      Reply ! {result, Block},
...
get_head(Reply, Storage) ->
    spawn(fun () ->
		  ...
		  receive
		    {head, Nonce, none} ->
			io:format("Got none as a head~n"),
			Reply ! {result, none};
		    {head, Nonce, Block} ->
			case check_block(Block) of
			  true ->
...
loop(Nodes, Nonces) ->
    receive
	  ...
	  loop(Nodes,
	       Nonces ++
		 nonces_cleaner(self(), [get_friends(self(), Node)]));
	  ...
	  Sender ! {friends, Nonce, Nodes}, loop(Nodes, Nonces);
	  ...
	  {New_nodes, New_nonces} = case lists:member({friends,
						       Nonce},
						      Nonces)
					of
				      true ->
					  {add_nodes(Incomming_nodes, Nodes),
					   Nonces -- [Nonce]};
				      false ->
      ...
      {push, Trans} ->
	  transaction_storage ! {add, Trans}, loop(Nodes, Nonces);
      % Update Block storage
      {update, Block} ->
	  head_storage ! {add, Block}, loop(Nodes, Nonces);
...
% Keep track of all blocks, block_storage behaves like a cache, when a block isn't stored it automaticaly requests it from a firend.
block_storage() ->
...
block_storage(Blocks) ->
    receive
	  ...
	  New_blocks = case is_new_block(Blocks, Block) of
			 true ->
			     {_, _, List_of_transaction, _} = Block,
			     transaction_storage !
			       {remove, List_of_transaction},
			     [Block | Blocks];
			 false -> Blocks
		       end,
	  block_storage(New_blocks);
      {clean, Head_id} ->
	  collect_chain(Head_id), block_storage(Blocks);
      {replace, New_blocks} -> block_storage(New_blocks);
      {get_all, Reply} ->
	  Reply ! {result, Blocks}, block_storage(Blocks);
      {get, Reply, Block_id} ->
	  get_block(Reply, Blocks, Block_id),
	  block_storage(Blocks);
...
collect_chain(Head_id) ->
    spawn(fun () -> collect_chain(Head_id, []) end).

collect_chain(Head_id, Blocks) ->
    case yield_block_by_id(Blocks, Head_id) of
      {Block_id, Prev_block_id, List, Solution} ->
	  collect_chain(Prev_block_id,
			{Block_id, Prev_block_id, List, Solution});
...
transaction_storage(Trans, Mining_trans) ->
    receive
	  ...
	  N_trans = case is_new_transaction(yield_blocks(),
					    Trans ++ Mining_trans, T)
			of
		      true -> % We need to gossip the new Transaction
			  (?NODE) ! {gossip, {push, T}}, [T | Trans];
		      false -> Trans
		    end,
	  {New_trans, New_mining_trans} = case length(N_trans) > 9
					      of
					    true ->
						{Block_trans, Remaing_trans} =
						    lists:split(10, N_trans),
						mine_block(Block_trans),
						{Remaing_trans,
						 Mining_trans ++ Block_trans};
					    false -> {N_trans, Mining_trans}
					  end,
	  transaction_storage(New_trans, New_mining_trans);
      {remove, T} ->
	  transaction_storage(Trans -- T, Mining_trans -- T)
    end.
...
head_storage(Heads) ->
    receive
	  ...
	  {Old_id, Old_length} = get_longest_head(Heads,
						  {none, 0}),
	  % We only need to store the longest head
	  New_heads = case Length > Old_length of
			true -> block_storage ! {clean, Id}, [{Id, Length}];
	  ...
	  head_storage(Heads);
      {get_longest, Reply} ->
	  get_chain(Reply, Heads), head_storage(Heads);
      {get, Reply, Block_id} ->
	  ...
	  io:format("respnded with this head ~p and block "
		    "~p~n",
		    [This_head, This]),
...
send_all([], _) -> none;
send_all([H | T], Data) ->
    msg_anomaly(H, Data), send_all(T, Data).
...
explore_chain({Head_id, Prev_head_id, _, _}) ->
    Main = self(),
    spawn(fun () ->
		  io:format("Start exploring chain ~p Prev: ~p~n",
...
explore_chain(Main, Head_to_insert, Prev_head_id,
	      Depth) ->
    case yield_head_by_id(Prev_head_id) of
      ...
      false ->
	  io:format("No head, looking at old blocks, explore "
		    "~p ~n",
		    [Prev_head_id]),
	  case yield_block_by_id(Prev_head_id) of
	    {_, Prev_block_id, _, _} ->
		io:format("Go deeper ~p~n", [Prev_block_id]),
		explore_chain(Main, Head_to_insert, Prev_block_id,
			      Depth + 1);
	    _ ->
		io:format("Block with id ~p doesn't exist. Drop "
			  "chain.~n",
			  [Prev_head_id])
	  end
    end.
...
yield_longest_head() ->
    ...
    receive
      {result, timeout} ->
	  io:format("get head timeout"), yield_longest_head();
      {result, failed} ->
	  io:format("No valid block as head: replying with "
		    "none"),
	  none;
      {result, R} -> R
    end.
...
yield_block_by_id(Block_id, Try) ->
    ...
    receive
      ...
      {result, failed} ->
	  retry(Try, yield_block_by_id(Block_id, Try + 1));
      {result, R} -> R
    end.
...
mine_block(Transactions) ->
    spawn(fun () ->
		  ...
		  Block_id = make_ref(),
		  head_storage !
		    {add, {Block_id, Id, Transactions, Solution}}
	  end).

% Checks if block is valid
check_block({_, Prev_block_id, List_of_transection,
	     Solution}) ->
    io:format("Check block~p~n",
	      [{Prev_block_id, Solution}]),
    proof_of_work:check({Prev_block_id,
			 List_of_transection},
			Solution).
...
is_new_transaction_blocks([{_, _, List_of_transection,
			    _}
			   | T],
			  {Trans_id, Payload}) ->
    case lists:keyfind(Trans_id, 1, List_of_transection) of
      false ->
	  is_new_transaction_blocks(T, {Trans_id, Payload});
      true -> false
    end.
...
get_random_friend_or_teacher(Nodes) ->
    get_random_friend(Nodes).
...
test_transaction() ->
    jsparber_node !
      {push, {make_ref(), "Some random data"}}.
